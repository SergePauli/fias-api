...
CREATE TABLE del_Houses_temp AS SELECT * FROM houses WHERE HOUSEID IS NULL;
...
/****************************************************************/
/* Загрузка во временную таблицу del_Houses_temp удалений       */
/* в основном списке  домов ФИАС                                */
/****************************************************************/
COPY  del_Houses_temp(AOGUID,BUILDNUM,ENDDATE,ESTSTATUS,HOUSEGUID,HOUSEID,
HOUSENUM,STATSTATUS,IFNSFL,IFNSUL,OKATO,OKTMO,POSTALCODE,STARTDATE,
STRUCNUM,STRSTATUS,TERRIFNSFL,TERRIFNSUL,UPDATEDATE,NORMDOC,COUNTER,
CADNUM,DIVTYPE) 
FROM 'absPATH2'
...
UPDATE houses h SET 
 ...
 /******************************************************************/
/* Удаление существующих записей основного списка houses  записей, */
/* на основании данных из временной таблицы del_Houses_temp        */
/*******************************************************************/

DELETE FROM houses h WHERE EXISTS(SELECT 1 FROM 
del_Houses_temp delh WHERE delh.HOUSEID=h.HOUSEID); 
...
DROP TABLE IF EXISTS Houses_temp;
DROP TABLE IF EXISTS del_Houses_temp;
